...
def create_vectorstore(_embeddings_model):
    return FAISS.load_local('db/faiss', _embeddings_model, allow_dangerous_deserialization=True)

@st.cache_resource
def setup_qa_chain(_vectorstore):
    system_template = """
    You are a super kind chatbot that provides information about Yonsei University.
    Focus ONLY on departments or majors provided. IF you don't know the answer, just say "I don't know".
            ...
            if result.get('source_documents'):
                ...
                for doc in result['source_documents']:
                    if hasattr(doc, 'page_content'):
                        ...
                        formatted_content = content.replace('\ ', '<br>').replace('\n', '<br>')
                        st.markdown(
                            f"""
